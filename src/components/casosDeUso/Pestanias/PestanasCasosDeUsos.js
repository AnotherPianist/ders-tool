...
export default function PestanasCasosDeUsos(props) {
  ...
  const [pestanias, setPestanias] = useState(
    isEmpty(props.casosDeUso) || props.casosDeUso === undefined ? dataDefault : props.casosDeUso.pestanias
    /* Aqui deberia recibir el prop de casos de usos guardados anteriormente */
  );
  const [casosDeUsos, setCasosDeUsos] = useState(
    isEmpty(props.casosDeUso) || props.casosDeUso === undefined ? [] : props.casosDeUso.diagramas
    /* Aqui deberia recibir el prop de casos de usos guardados anteriormente */
  );
    ...
    async (event, indiceTab) => {
      ...
      const cerrar = async () => {
        if (cerrarPestania) {
          ...
          props.actualizarCasosDeUso({pestanias: copiaPestanias, diagramas: copiaDiagramas});
        }
      };
  ...
  const nuevaPestania = (nombre, diagrama, posicion, clave) => {
    ...
    props.actualizarCasosDeUso({pestanias: copiaPestanias});
  ...
  const cargarDiagrama = (diagrama, posicion) => {
    ...
    const copiaCasosDeUso = casosDeUsos.slice(0, posicion).concat({
      nombre: diagrama.nombre,
      diagrama: diagrama.diagrama,
      activo: true,
      key: diagrama.key,
    }).concat(casosDeUsos.slice(posicion + 1, casosDeUsos.length));
    setCasosDeUsos(copiaCasosDeUso);
    // props.actualizarCasosDeUso({diagramas: copiaCasosDeUso});
  };
  ...
  const nuevoDiagrama = () => {
    ...
    const copiaCasosDeUso = casosDeUsos.concat({
  ...
  const actualizarEstado = (casoDeUSo) => {
    ...
    // props.actualizarCasosDeUso({pestanias: copiaPestanias, diagramas: diagramas});
  };
  ...
  const actualizarDiagramas = (diagramas) => {
    setCasosDeUsos(diagramas);
    props.actualizarCasosDeUso({diagramas: diagramas});
  };
